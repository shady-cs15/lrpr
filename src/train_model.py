...
valid_set = ()
test_set = ()
for f in image_files[:n_images]:   #TODO- remove restriction
...
for f in image_files[n_images:n_images+50]:   #TODO- remove restriction
	img = Image.open(f).convert('L')
	img = np.array(img, dtype='float64') / 256.
	img = img.reshape(1, 1, img_h, img_w)
	valid_set+=(img, )
...
def pretrain_nnet(data_set, n_images=100, batch_size=5, learning_rate=0.07):
	# pass the entire dataset into pretrain_conv_autoencoder function
	# not individual mini_batches
	# the function should implement stochastic gradient descent conisdering 
	# mini_batches on its own
	# change function arguments of the method
	layer0 = data_set
	layer1 = pretrain_conv_autoencoder(1, layer0, (batch_size, 1, 96, 336), (2, 1, 3, 3), 5, (1, 1), learning_rate)
	layer2 = pretrain_conv_autoencoder(2, layer1.output, (batch_size, 2, 96, 336), (3, 2, 3, 3), 5, (2, 2), learning_rate)
	layer3 = pretrain_conv_autoencoder(3, layer2.output, (batch_size, 3, 48, 168), (5, 3, 3, 3), 5, (1, 1), learning_rate)
	layer4 = pretrain_conv_autoencoder(4, layer3.output, (batch_size, 5, 48, 168), (8, 5, 3, 3), 5, (2, 2), learning_rate)
	layer5 = pretrain_conv_autoencoder(5, layer4.output, (batch_size, 8, 24, 84), (10, 8, 3, 3), 5, (2, 2), learning_rate)
	# TODO - layer 8 - layer 12 should use pretrain_deconv_autoenc 
	# because of unpooling occurs before deconv
	# opposite to the case of convolution 
	# or change pretrain_conv_autoencoder to include unpooling
	# before deconvolution
	layer8 = pretrain_conv_autoencoder(8, layer5.output, (batch_size, 10, 12, 42), (8, 10, 3, 3), 5, (2, 2), learning_rate, unpool=True, switch=layer5.switch)
	layer9 = pretrain_conv_autoencoder(9, layer8.output, (batch_size, 8, 24, 84), (5, 8, 3, 3), 5, (2, 2), learning_rate, unpool=True, switch=layer4.switch)
	layer10 = pretrain_conv_autoencoder(10, layer9.output, (batch_size, 5, 48, 168), (3, 5, 3, 3), 5, (1, 1), learning_rate)
	layer11 = pretrain_conv_autoencoder(11, layer10.output, (batch_size, 3, 48, 168), (2, 3, 3, 3), 5, (2, 2), learning_rate, unpool=True, switch=layer2.switch)
	layer12 = pretrain_conv_autoencoder(12, layer11.output, (batch_size, 2, 96, 336), (1, 2, 3, 3), 5, (1, 1), learning_rate)
	
	return layer1.params, layer2.params, layer3.params, layer4.params, layer5.params, layer5.params, layer5.params, layer8.params, layer9.params, layer10.params, layer11.params, layer12.params


def train_nnet(rng, data_set, n_examples, batch_size=5, learning_rate=0.07, init=False, params=None):

	x = T.tensor4('x')#.reshape((batch_size, 1, data_set[0].shape.eval()[2], data_set[0].shape.eval()[3]))
	index = T.lscalar()
...
train_nnet(rng, (train_set, valid_set), (200, 50), init=True, params=params)
#print T.sum(T.sqr(a1-a2)).eval()